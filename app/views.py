...
from app.models import Project, ContactMessage, Subscriber
...
def home(request):
    ...
    # context["snum"]="slide"
    # context["s"]= "service__description  service__description--active  slide"
    ...
    if request.method == 'POST':
        ...
        if not errors:
            try:
                ...
                contct = ContactMessage.objects.create(name=request.POST['name'], subject=request.POST['subject'],
                                                       email=request.POST['email'], enquiry=request.POST['message'])
                contct.save()
                msg = "\r\n".join([
                    "From: arabic.kanz.contact@gmail.com",
                    "To: info@tamkeen4st.com",
                ...
                server.sendmail(EMAIL_HOST_USER, 'info@tamkeen4st.com', msg)
                server.quit()
